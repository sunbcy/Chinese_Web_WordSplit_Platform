...
def wash_html(data, url=''):  # 提取网页中主要含有的汉字
    bcy_stop_words_punctuations = open(os.path.abspath('Bcy_Stopwords/Bcy_Stopwords_Punctuations'), 'r',
                                       encoding='utf-8').read()
    bcy_stop_words_punctuations = [i for i in bcy_stop_words_punctuations.split('\n') if i]
    web_info = {
        'url': url,
        'title': '',
    ...
    for node in all_nodes:
        ...
        if (('<script' in node_content and '</script>' in node_content) or
                ('<script src="' in node_content and '.js' in node_content) or
                ('<style' in node_content and '</style>' in node_content) or
                ('<applet' in node_content and '</applet>' in node_content)):
            pass
        ...
        # 提取meta
        elif (('<meta' in node_content and 'name="keywo' in node_content) or
              ('<meta' in node_content and 'name="description"' in node_content)):
        ...
        elif ('<li' in node_content and (node.text != "" or node.xpath('p//text()') != [] or
                                         node.xpath('a//text()') != [])):
        ...
        elif '<div ' in node_content and (node.xpath('p//text()') != [] or
                                          node.xpath('span//text()') != []):
    ...
    title_dict = {}
    title_list = []
    title_dict[web_info['title']] = [i for i in word_cut2partofspeech(web_info['title']) if
                                     i not in bcy_stop_words_punctuations]  # 过滤停用词标点符号
    ...
    for w in web_info['meta_keywords_description']:
        meta_dict = {w: [i for i in word_cut2partofspeech(w) if i not in bcy_stop_words_punctuations]}
    ...
    for w in web_info['a_text']:
        a_text_dict = {}
        if len(w) > 6:
            a_text_dict[w] = [i for i in word_cut2partofspeech(w) if i not in bcy_stop_words_punctuations]  # 过滤停用词标点符号
        else:
            a_text_dict[w] = [w]
    ...
    for w in web_info['text']:
        text_dict = {}
        if len(w) > 6:
            text_dict[w] = [i for i in word_cut2partofspeech(w) if i not in bcy_stop_words_punctuations]  # 过滤停用词标点符号
        else:
            temp_text = [i for i in word_cut2partofspeech(w) if i not in bcy_stop_words_punctuations]
